...
Modified by Chris Cantrell to isolate serial functions allowing overrides
for over-the-air functions.
...
class Loader(object):
    ...
    def connect(self):
        ...
        for (i, value) in zip(range(lfsrRequestLen + lfsrReplyLen), lfsr(lfsrSeed)):
            seq.append(value)
        self.writeString("".join(chr(each | 0xfe) for each in seq[0:lfsrRequestLen]))
        self.writeString(chr(0xf9) * (lfsrReplyLen + 8))
        for i in range(lfsrRequestLen, lfsrRequestLen + lfsrReplyLen):
            if self.readBit(False, 0.500) != seq[i]:
    ...
    def serInit(self,baud,port):
        print baud
        self.serial = serial.Serial(baudrate=baud, timeout=0)
    ...
    def writeLong(self, value):
        ...
        self.writeString(encodeLong(value))
